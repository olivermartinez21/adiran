...
public interface ICatComponentRepository extends JpaRepository<CatComponentModel, String>{
	...
	@Query(value = "SELECT * FROM MYRE_CAT_COMPONENT where secction =:section and DC=:containerType or RF=:containerType or GD=:containerType or CH=:containerType ORDER BY COMPONENT ASC", nativeQuery = true)
	List<CatComponentModel> findByContainerTypeAndSecction(@Param("section") String section, String containerType);

	@Query(value = "SELECT DISTINCT SECCTION FROM MYRE_CAT_COMPONENT where DC=:containerType or RF=:containerType or GD=:containerType or CH=:containerType ORDER BY SECCTION ASC", nativeQuery = true)
