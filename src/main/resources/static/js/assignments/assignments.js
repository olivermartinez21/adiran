...
function configDataTable() {
	...
	$("#bookingInformation").DataTable({
		...
		select: { style: 'single' },
		buttons: {
			buttons: [ {text: '+', action: function() { addInformationBooking(); }} ],
			dom: { button:{ tag:"button", className:"btn btn-dark" } }
		},
		columns: [
	...
	if($(this).val().length==11){
		$.ajax({
			...
			success: function(response){
				...
				$("#gradeQuality").val($("#containerCondition option:selected").html()+" "+$("#containerClasificacion option:selected").html());
...
function bookingValidation(){
	var value = $("#newBooking").val();
	if(value.length == 8){ $("#newShippingConpanyBooking").val(3); }
	if(value.length == 9){ $("#newShippingConpanyBooking").val(2); }
	if(value.length == 14){ $("#newShippingConpanyBooking").val(6); }
}
