...
function configDataTable() {
	...
	$("#quoteTable").DataTable({
		...
		select: {
			...
			buttons: {
			buttons: [],
			dom: { button:{ tag:"button", className:"btn btn-dark" } }
		},
		ajax: {
			...
			dataSrc: '',
			data: {appointmentId:  $("#appointmentId").val(), userId: $("#globalUserId").val()},
			error: function(response) { manageErrorAjax(response); }
		},
		columns: [
...
function dataTableInpection() {
	...
	$("#inspectionTable").DataTable({
		dom: "<'row'<'col-sm-6 'l><'col-sm-4 '><'col-sm-2 dt-right'f>>" +
		"<'row'<'col-sm-12'B>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-5'i><'col-sm-7 text-right'p>>",
		fixedHeader: true, responsive: true, autoWidth: true,
		select: { style: 'single' },
		...
		columns: [
			...
			{ data: "component", visible: true, render: function(data) { $("#catComponent").val(data); return $("#catComponent option:selected").html(); }},
			{ data: "damage", visible: true, render: function(data) { $("#catDamage").val(data); return $("#catDamage option:selected").html(); }},
			{ data: "repair", visible: true, render: function(data) { $("#catRepair").val(data); return $("#catRepair option:selected").html(); }},
			{ data: "length", visible: true, render: function(data) { $("#length").val(data); return data; }},
			{ data: "width", visible: true, render: function(data) { $("#width").val(data); return data; }},
			{ data: "inspectionId", visible: true, render: function(data) { return $("#length").val() * $("#width").val(); }},
			{ data: "customerType", visible: true, render: function(data) { $("#inspectionCustomerType").val(data); return $("#inspectionCustomerType option:selected").html(); }},
			...
			{ data: "status", visible: false, render: function(data) { return data; }},
...
function initComponents(){
	$("#inspectionCapModelForm").submit(function () {
		var data = {
			...
			success: function(response){
				if(response.success===true){
					...
					.then(() => {
						dataTableInpection();
						configDataTable();
						var btAcept = document.getElementById("btAcept");
						if(response.num===0 && btAcept){
							btAcept.removeAttribute("hidden");
						}
					});
				}else{
	...
	$("#quoteModelForm").submit(function () {
		$.ajax({
			type: "POST",
			url: 'quoteClients/changeStatus',
			cache: false,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			data: {containerId: $("#containerId").val(), coments: $("#coments").val(), status: 3},
			success: function(response){
				if(response.success===true){
					Swal.fire("Proceso Exitoso", "", "success")
					.then(() => { $("#quoteModel").modal("hide"); configDataTable(); });
				}else{
					Swal.fire(response.message+" Error", "", "warning");
				}
			},
	...
	$("#denegateForm").submit(function () {
		$.ajax({
			type: "POST",
			url: 'quoteClients/changeStatus',
			cache: false,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			data: {containerId: $("#containerId").val(), coments: $("#coments").val(), status: $("#option").val()},
			success: function(response){
				if(response.success===true){
					Swal.fire("Proceso Exitoso", "", "success")
					.then(() => { $("#denegateModel").modal("hide"); $("#quoteModel").modal("hide"); configDataTable(); });
				}else{
					Swal.fire(response.message+" Error", "", "warning");
				}
			},
...
function viewPhotos(data){
	...
	$.ajax({
		...
		contentType: "application/x-www-form-urlencoded; charset=UTF-8",
		data: {containerId: $("#inspectionId").val()},
		success: function(response){
...
function showComponents(){
	...
	$.ajax({
		...
		contentType: "application/x-www-form-urlencoded; charset=UTF-8",
		data: {containerType: textContainer, Section: data},
		success: function(response){
